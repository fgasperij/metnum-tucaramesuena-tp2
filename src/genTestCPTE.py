...
# Parametro 6 (opcional) = Semilla, si no se proporciona ninguna, la semilla es 
# "aleatoria", por lo que cada llamada al programa arrojará diferentes instancias
def main(arg = argv):
...
# En principio testeo con todas las imagenes que no estan cargadas, quizas 
# despues se pueda parametrizar.	
	f.write( str(personas) )
	f.write("\n")

	for i in range(end_pers+1):
		if (orden[i] != 0):
			j = randint(start_ipp, end_ipp)
			while (Matriz[i][j] == 1):
				j = randint(start_ipp, end_ipp)
